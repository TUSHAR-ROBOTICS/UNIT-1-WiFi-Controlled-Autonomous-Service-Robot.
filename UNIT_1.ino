...
const char* password = "UNIT-1CODE";
...
// ── Config ────────────────────────────────────────────────────
int motorSpeed                = 150;
#define OBSTACLE_DISTANCE_AUTO  20
#define OBSTACLE_DISTANCE_SMART 30
...
// ═════════════════════════════════════════════════════════════
void runSmartMode() {
  if (smartAvoiding) { runAutoState(OBSTACLE_DISTANCE_SMART); return; }
  currentStatus = "SMART";
...
void setupRoutes() {
  ...
  server.on("/mode", []() {
    if (server.hasArg("m")) {
      ...
      smartAvoiding = false; robotState = STATE_FORWARD;
...
void updateDisplay() {
  ...
  bool inv = (currentStatus == "OBSTACLE!" || currentStatus == "E-STOP" ||
              currentStatus == "WARNING");
  if (inv) {
    display.fillRect(48, 0, 80, 9, WHITE);
    display.setTextColor(BLACK); display.setCursor(50, 0);
    display.print(currentStatus); display.setTextColor(WHITE);
  ...
  display.setCursor(0, 33); display.print("SPD : ");
  display.print(motorSpeed); display.print(" / 1023");
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  body::before{
    ...
    background:repeating-linear-gradient(
      0deg,rgba(0,0,0,0.018) 0px,rgba(0,0,0,0.018) 1px,
  ...
  /* HEADER */
  .header{
    border:2px solid #0a0a0a; padding:8px 14px;
    display:flex; align-items:center; justify-content:space-between;
    background:#ffffff;
  }
  .header-title{
    font-family:'Oswald',sans-serif;
    font-size:1.05rem; font-weight:600; letter-spacing:5px; color:#0a0a0a;
  }
  .header-sub{ font-size:0.6rem; color:#555; letter-spacing:2px; margin-top:2px; }
  .blink{ animation:blink 1s step-end infinite; }
  @keyframes blink{ 50%{ opacity:0; } }

  /* SECTION LABEL */
  .slbl{
    font-size:0.58rem; letter-spacing:3px; color:#555;
  ...
  /* OLED */
  .oled-frame{ border:2px solid #0a0a0a; padding:1px; background:#fff; }
  .oled-tb{
  ...
  .oled-in{
    background:#f5f7fa; border:1px solid #ccc;
    padding:10px 12px; font-size:0.7rem; line-height:1.9; color:#0a0a0a;
  }
  .oled-r{ display:flex; justify-content:space-between; align-items:center; }
  .oled-l{ color:#666; min-width:52px; }
  .oled-v{ color:#0a0a0a; font-weight:bold; }
  .odiv{ border-top:1px solid #bbb; margin:5px 0; }
  .eyes{ display:flex; justify-content:space-between; padding:5px 8px 0; gap:8px; height:22px; }
  .eye{ background:#0a0a0a; border-radius:3px; flex:1; transition:all 0.2s; }
  .eye.angry{ clip-path:polygon(0 45%,50% 0%,100% 0%,100% 100%,0 100%); }
  .eye.dead{
    background:transparent; border:2px solid #0a0a0a; position:relative;
  }
  .eye.dead::before,.eye.dead::after{
    content:''; position:absolute; top:50%; left:50%;
    width:100%; height:2px; background:#0a0a0a;
  }
  .eye.dead::before{ transform:translate(-50%,-50%) rotate(35deg); }
  .eye.dead::after{  transform:translate(-50%,-50%) rotate(-35deg); }
  .hinv{ background:#0a0a0a; color:#fff; padding:0 5px; font-weight:bold; }

  /* SENSOR PANEL */
  .radar-frame{ border:2px solid #0a0a0a; padding:1px; background:#fff; }
  .radar-tb{
    background:#0a0a0a; color:#fff;
    font-family:'Oswald',sans-serif; font-size:0.62rem;
    letter-spacing:3px; padding:3px 10px;
  ...
  .rarc1{ width:84px; height:84px; border:1px solid #bbb; }
  .rarc2{ width:56px; height:56px; border:1px solid #aaa; }
  .rarc3{ width:28px; height:28px; border:1px solid #999; }
  .rsweep{
    position:absolute; bottom:0; left:50%;
    width:1px; height:42px; background:#0a0a0a;
  ...
  .rstat-l{ color:#666; }
  .rstat-v{ color:#0a0a0a; font-weight:bold; }
  .dbar-wrap{ width:100%; height:7px; background:#ddd; border:1px solid #bbb; margin-top:2px; }
  .dbar{ height:100%; background:#0a0a0a; transition:width 0.35s; }
  .obj-badge{
    font-size:0.58rem; letter-spacing:2px;
    border:1px solid #888; color:#555;
  ...
  .zone{ font-size:0.55rem; padding:2px 6px; border:1px solid #ccc; color:#999; letter-spacing:1px; }
  .zone.hot{ border-color:#0a0a0a; background:#0a0a0a; color:#fff; }

  /* MODE BUTTONS */
  .mrow{ display:flex; gap:7px; }
  .mbtn{
    flex:1; padding:11px 4px;
    background:#fff; border:2px solid #bbb; color:#777;
    font-family:'Share Tech Mono',monospace;
    font-size:0.63rem; letter-spacing:1px;
    cursor:pointer; text-transform:uppercase;
    transition:all 0.15s; line-height:1.7;
  }
  .mbtn:hover{ border-color:#0a0a0a; color:#0a0a0a; }
  .mbtn.act{ background:#0a0a0a; color:#fff; border-color:#0a0a0a; }

  /* D-PAD */
  .dpad{
    border:2px solid #ccc; padding:18px; background:#fff;
    display:flex; flex-direction:column; align-items:center; gap:7px;
  }
  .drow{ display:flex; gap:7px; align-items:center; }
  .db{
    width:78px; height:78px;
    background:#fff; border:2px solid #bbb; color:#0a0a0a;
    font-size:1.5rem;
    display:flex; align-items:center; justify-content:center;
    cursor:pointer; transition:all 0.1s; user-select:none;
  }
  .db:hover{ border-color:#0a0a0a; background:#f0f0f0; }
  .db:active,.db.active{ background:#0a0a0a; color:#fff; border-color:#0a0a0a; }
  .db.disabled{ opacity:0.2; pointer-events:none; }
  .dc{
    width:78px; height:78px;
    background:#f5f7fa; border:2px solid #ddd;
    display:flex; align-items:center; justify-content:center;
    font-size:0.55rem; color:#bbb; letter-spacing:1px;
  }
  ...
  #smart-notice{
    ...
    padding:6px; animation:blink 0.8s step-end infinite;
    margin-top:6px;
  }
  ...
  #btn-ESTOP:hover{ background:#0a0a0a; color:#fff; }
  ...
  #btn-RESET:hover{ background:#0a0a0a; color:#fff; }

  /* FOOTER */
  .foot{
    border-top:2px solid #ccc; padding-top:8px;
    font-size:0.55rem; color:#999; letter-spacing:1px;
...
<div class="header full">
  ...
  <div style="font-size:0.62rem;color:#666;text-align:right;line-height:1.8;">
    ...
    <span style="color:#aaa;font-size:0.54rem;" id="h-mode">STANDBY</span>
  </div>
</div>
    ...
    <div class="radar-frame">
      ...
      <div class="radar-body">
        ...
        <div class="radar-stats">
          ...
          <div class="rstat"><span class="rstat-l">THRESHOLD</span><span class="rstat-v" id="r-thresh">30 cm</span></div>
          <div class="zones">
...
function updateUI(d){
  ...
  if(d.obstacle){
    ...
    if(dist<10){ rSt.textContent='!! CRITICAL !!'; zC.classList.add('hot'); }
    else        { rSt.textContent='!! WARNING !!';  zW.classList.add('hot'); }
  } else {
    badge.textContent='CLEAR'; badge.className='obj-badge';
    dot.style.display='none'; rSt.textContent='SCANNING'; zS.classList.add('hot');
  }
